...
export const deadSubbots = new Set()
...
export async function startSubBot(m, parentConnection, caption = '', isCode = false, phone = '', chatId = '', commandFlags = {}) {
  ...
  try {
    ...
    if (m) {
      let credsMuertas = deadSubbots.has(id)
      const credsFile = path.join(sessionFolder, 'creds.json')
      if (!credsMuertas && fs.existsSync(credsFile)) {
        try {
          credsMuertas = !JSON.parse(fs.readFileSync(credsFile, 'utf8'))?.registered
        } catch {
          credsMuertas = true
        }
      }
    ...
    socket.ev.on('connection.update', async ({ connection, lastDisconnect, qr }) => {
      ...
      if (connection === 'close') {
        ...
        if ([401, 403, DisconnectReason.loggedOut, DisconnectReason.connectionReplaced].includes(reason)) {
          retryCounters.delete(id)
          deadSubbots.add(id)
          console.error(`[SUB-BOT ${id}] Sesion no autorizada o reemplazada (codigo ${reason}). Se conserva sin reintentar; con .code o .serbot se vincula de nuevo.`)
          return
        }
