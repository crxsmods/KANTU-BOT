...
newsletterJid: (() => {
  const loli = "120363371008200788@newsletter";
  const otros = ["120363178718483875@newsletter", "120363178718483875@newsletter"];
...
const defaultContextInfo = async (caption, conn) => ({
  ...
  forwardedNewsletterMessageInfo: {
    newsletterJid: (() => {
  const loli = "120363371008200788@newsletter";
  const otros = ["120363178718483875@newsletter", "120363371008200788@newsletter"];
  return Math.random() < 0.75 ? loli :  otros[Math.floor(Math.random() * otros.length)]})(),
   newsletterName: "Hack Store X 💫"
  }
});
