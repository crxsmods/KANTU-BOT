...
const photoSetting = hasFoto ? true : hasNoFoto ? false : null
await db.query(`INSERT INTO group_settings (group_id) VALUES ($1) ON CONFLICT DO NOTHING`, [m.chat])

if (command === 'setwelcome') {
await db.query(
  'UPDATE group_settings SET swelcome = $1, photowelcome = COALESCE($2::boolean, photowelcome) WHERE group_id = $3',
  [cleanText, photoSetting, m.chat]
)
...
if (command === 'setbye') {
await db.query(
  'UPDATE group_settings SET sbye = $1, photobye = COALESCE($2::boolean, photobye) WHERE group_id = $3',
  [cleanText, photoSetting, m.chat]
)
