...
const newsletters = [
  { jid: '120363371008200788@newsletter', name: 'Hack Store X 💫', weight: 30 },
  { jid: '120363418584876374@newsletter', name: 'Kantu Bot Ofc ⚡', weight: 10 },
  { jid: '120363178718483875@newsletter', name: 'The Kantu Bot 🔥', weight: 30 },
  { jid: '120363417790731866@newsletter', name: 'KantuBot Test ✨', weight: 30 }
]

const pickNewsletter = () => {
  const total = newsletters.reduce((sum, n) => sum + n.weight, 0)
...
export const randomNewsletterInfo = () => {
  const n = pickNewsletter()
  return { newsletterJid: n.jid, newsletterName: n.name }
}
...
const enforceNewsletterPolicy = contextInfo => {
  ...
  if (!info) return
  if (!/^\d{10,30}@newsletter$/.test(String(info.newsletterJid || ''))) {
