...
// ───────────────────────────────────────────────────────────────────────
// Normaliza y valida DATABASE_URL sin nunca imprimir su valor real.
// Corrige errores comunes de copia/pegado (comillas envolventes, espacios
// o saltos de línea accidentales) y valida la estructura básica antes de
// pasarla a "pg", para dar un mensaje de error claro en vez del genérico
// "Invalid URL" de Node cuando algo está mal formado.
// ───────────────────────────────────────────────────────────────────────
function normalizeDatabaseUrl(raw) {
  if (typeof raw !== 'string') return '';
  let value = raw.trim();
  // Quita comillas envolventes accidentales: DATABASE_URL="postgres://..."
  if (
...
if (!DATABASE_URL) {
  throw new Error(
    '[DB] ❌ La variable de entorno DATABASE_URL no está definida (o quedó vacía).\n' +
...
if (!/^postgres(ql)?:\/\//i.test(DATABASE_URL)) {
  throw new Error(
    '[DB] ❌ DATABASE_URL no empieza con "postgres://" o "postgresql://".\n' +
    '     Revisa que copiaste la cadena de conexión completa (no solo el host o la contraseña).'
  );
}

if (/\s/.test(DATABASE_URL)) {
  throw new Error(
    '[DB] ❌ DATABASE_URL contiene espacios o saltos de línea.\n' +
    '     Revisa el archivo .env: la cadena debe estar en una sola línea, sin espacios sueltos.\n' +
    '     Si tu contraseña tiene un espacio real, debe ir codificada como %20.'
  );
}

try {
  // Valida que la URL sea parseable ANTES de dársela a pg. Si esto falla,
  // casi siempre es porque la contraseña (u otro segmento) tiene
  // caracteres especiales (# @ : / %) sin codificar con percent-encoding.
  // eslint-disable-next-line no-new
  new URL(DATABASE_URL);
} catch (err) {
  throw new Error(
    '[DB] ❌ DATABASE_URL no es una URL válida (no se imprime el valor por seguridad).\n' +
    '     Causa más común: la contraseña tiene caracteres especiales sin codificar.\n' +
    '     Si tu contraseña incluye # @ : / % espacios, etc., debes codificarlos con\n' +
    '     percent-encoding, por ejemplo: @ → %40, # → %23, : → %3A, / → %2F, % → %25.\n' +
    '     Ejemplo: postgres://usuario:mi%40clave%23123@host:5432/basededatos'
  );
}
