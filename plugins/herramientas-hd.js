...
if (!info.neoxr.key) return m.reply('❌ La mejora HD está desactivada: configura NEOXR_API_KEY.')
let res = await fetch(`${info.neoxr.url}/remini?image=${encodeURIComponent(url)}&apikey=${encodeURIComponent(info.neoxr.key)}`)
