...
try {
if (!info.zenkey.key) throw new Error('Zenkey no configurado')
const res = await fetch(`${info.zenkey.url}/download/ytmp3?apikey=${encodeURIComponent(info.zenkey.key)}&url=${encodeURIComponent(args)}`)
...
try {
if (!info.zenkey.key) throw new Error('Zenkey no configurado')
const res = await fetch(`${info.zenkey.url}/download/ytmp4?apikey=${encodeURIComponent(info.zenkey.key)}&url=${encodeURIComponent(args)}`)
let { result } = await res.json()
